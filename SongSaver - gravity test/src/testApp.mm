...
void testApp::setup(){
    ...
    enableGridButt.setup(); 
    enableGridButt.setColor(ofColor(232, 58, 37));
    enableGridButt.setLabel("GRID", &whitneySemiBold22);
    
    enablePathButt.setup(); 
    enablePathButt.setColor(ofColor(232, 58, 37));
    enablePathButt.setLabel("GRAVITY", &whitneySemiBold22);
...
void testApp::update(){
    ...
    if (begin) {
        ...
        //audio controls gravity
        float mapPath = ofClamp(mapPath, 50, 100);
        mapPath = ofMap(theBin, 50, 100, 40, -40);    
        mapPath = ofClamp(mapPath, -40, 40);
        goBall = mapPath; 
        //if (enablePathMe) songPath.setGravity(mapPath, mouseMove);
        ofSetColor(255, 0, 255);
        ofDrawBitmapString("Bin: " + ofToString(theBin), 100, ofGetHeight()-100);
        ofDrawBitmapString("Gravity " + ofToString(mapPath), 100, ofGetHeight()-85);
    ...
    if (enablePathMe) songPath.updatePath(); 
}
...
void testApp::draw(){
    ...
    enableGridButt.draw(600, 10); 
    enablePathButt.draw(600, 50); 
    ...
    if (begin && theBins.size() != 0) {
        ...
        for (int i = 0; i < theBins.size()-1; i++){
            if (enableGridMe) grid.letsGo(theBins.size(), theBins[i]);
            
            songPath.moveBall(grid.startLoc.x + ((grid.lenHorz/grid.numHorz)* (i)), goBall);
            
            /*
            float mapPath = ofMap(theBins[i], 30, 128, 5, -5);
            mapPath = ofClamp(mapPath, -20, 20);
            
            if (enablePathMe) songPath.setGravity(mapPath);
             */
    ...
    if (enablePathMe) songPath.drawPath(); 
...
void testApp::touchDown(ofTouchEventArgs &touch){
    ...
    enableGridButt.touchDown(touch);
    enablePathButt.touchDown(touch);
    
    songPath.touchDown(touch);
}

//--------------------------------------------------------------
void testApp::touchMoved(ofTouchEventArgs &touch){
    /*
    float mapPath = ofMap(touch.y, 0, ofGetHeight(), 20, -20);
    mapPath = ofClamp(mapPath, -20, 20);    
    if (enablePathMe) songPath.setGravity(mapPath);
     */
    
    mouseMove = ofMap(touch.x, 0, ofGetWidth(), -2, 2);
...
void testApp::touchUp(ofTouchEventArgs &touch){
    ...
    if (enableGridButt.isPressed()) {
        enableGridMe = true;
        enablePathMe = false; 
    }
    
    if (enablePathButt.isPressed()) {
        enableGridMe = false;
        enablePathMe = true; 
    }
    ...
    enableGridButt.touchUp(touch);
    enablePathButt.touchUp(touch);
    
    songPath.touchUp(touch); 
}
