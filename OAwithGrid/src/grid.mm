...
void grid::setupGrid() {
    ...
    for (int i=0; i<numHorz-1; i++) {
        ofxBox2dCircle topAnchor;
		topAnchor.setup(box2d.getWorld(), startLoc.x + ((lenHorz/numHorz)* (i+1)), startLoc.y - anchorHeight, 2); 
        topAnchor.setFixedRotation(false);
		topAnchors.push_back(topAnchor);
    ...
    for (int i = 0; i < topCircles.size(); i++) {
        ...
        if (i == 0 ) {
            ...
            topAnchorJoint.setup(box2d.getWorld(), topAnchors[i].body, topCircles[i].body); 
        }
        ...
        topAnchorJoint.setLength(anchorHeight);
        topAnchorJoints.push_back(topAnchorJoint);
    }
...
void grid::letsGo(int num, int height){
    ...
    topAnchorJoints[numNote].setLength((anchorHeight) - height);
}